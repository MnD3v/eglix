...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3 m-0">Modifier don</h1>
        <a href="{{ route('donations.index') }}" class="btn btn-outline-secondary">Retour</a>
    </div>
    <form method="POST" action="{{ route('donations.update', $donation) }}" class="card card-soft p-3">
        @csrf
        @method('PUT')
        <div class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-3" id="amount-field">
                ...
                <input type="number" step="0.01" name="amount" value="{{ old('amount', $donation->amount) }}" class="form-control @error('amount') is-invalid @enderror">
            ...
            <div class="col-12" id="physical-description-field" style="display: none;">
                ...
                <textarea name="physical_description" class="form-control @error('physical_description') is-invalid @enderror" rows="2" placeholder="Décrivez l'objet donné...">{{ old('physical_description', $donation->physical_description) }}</textarea>
            ...
            <div class="col-md-6">
                ...
                <input name="donor_name" value="{{ old('donor_name', $donation->donor_name) }}" class="form-control @error('donor_name') is-invalid @enderror">
                @error('donor_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-4" id="reference-field">
                <label class="form-label">Référence</label>
                <input name="reference" value="{{ old('reference', $donation->reference) }}" class="form-control @error('reference') is-invalid @enderror">
                @error('reference')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-12">
                <label class="form-label">Notes</label>
                <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3">{{ old('notes', $donation->notes) }}</textarea>
