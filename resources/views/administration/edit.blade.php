...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Modifier la fonction administrative</h1>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card card-soft">
                <div class="card-body">
                    <form action="{{ route('administration.update', $administration) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <select name="member_id" id="member_id" class="form-select @error('member_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($members as $member)
                                        <option value="{{ $member->id }}" {{ old('member_id', $administration->member_id) == $member->id ? 'selected' : '' }}>
                                ...
                                @if(count($functions) > 0)
                                    <select name="function_name" id="function_name" class="form-select @error('function_name') is-invalid @enderror" required>
                                        ...
                                        @foreach($functions as $key => $value)
                                            <option value="{{ $key }}" {{ old('function_name', $administration->function_name) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                @else
                                    ...
                                    <input type="text" name="function_name" id="function_name" class="form-control @error('function_name') is-invalid @enderror" 
                                           value="{{ old('function_name', $administration->function_name) }}" placeholder="Nom de la fonction" required>
                                @endif
                                @error('function_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="start_date" id="start_date" 
                                       ...
                                       value="{{ old('start_date', $administration->start_date->format('Y-m-d')) }}" required>
                                @error('start_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="end_date" id="end_date" 
                                       ...
                                       value="{{ old('end_date', $administration->end_date?->format('Y-m-d')) }}">
                            ...
                            <div class="col-12">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" 
                                           value="1" {{ old('is_active', $administration->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="notes" id="notes" rows="3" 
                                          ...
                                          placeholder="Notes supplémentaires sur cette fonction...">{{ old('notes', $administration->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
